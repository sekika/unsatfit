...
def main():
    ...
    if args.model:
        models = [args.model]
    test_models(unsoda, ids, models, args.verbose)


def test_models(unsoda, ids, models, verbose):
    import unsatfit
    tested = 0
    for id in ids:
        tested += 1
        ...
        f.header = f'======== UNSODA {id} {texture} ========\n'
        f.header += f'https://seki.webmasters.gr.jp/swrc/?unsoda={id}&place=lab&process=drying'
        if verbose > 1:
            print(f.header)
        for model in models:
            if verbose == 1:
...
def test_model(f, model, verbose):
    qs = max(f.swrc[1])
    const_ini, min_r2_init, min_r2 = get_model_setting(model, qs)
...
def get_model_setting(model, qs):
    ...
    return const_ini, min_r2_init, min_r2
