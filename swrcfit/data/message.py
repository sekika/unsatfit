def message(lang, ID, URL='./'):
    ...
    if ID == 'ack':
        ...
        if lang == 'zh':
            return '<h2>感谢</h2><ul><li>Sample data is from <a href="http://www.ars.usda.gov/Services/docs.htm?docid=8967">UNSODA database</a> of unsaturated soil hydraulic properties developed by US Salinity Laboratory.</li>' \
    ...
    if ID == 'question':
        if lang == 'ja':
            return '<h2>質問</h2><p>質問は<a href="https://github.com/sekika/unsatfit/issues?q=is%3Aissue">unsatfitリポジトリのissues</a>に送ってください。GitHub のアカウントを取得しサインインして、緑色の「New Issue」ボタンを押して下さい。日本語での質問も可能です。コメントで返信します。未公開のデータに関する質問など、質問を公開出来ない場合には、メールで質問をしてください。メールアドレスは<a href="https://doi.org/10.1002/vzj2.20168">この論文</a>にあります。</p>'
        return '<h2>Question</h2><p>Please send questions and bug reports to <a href="https://github.com/sekika/unsatfit/issues?q=is%3Aissue">issues of unsatfit repository</a>. Get GitHub account, sign in, and press the green "New Issue" button. I will reply by comment. It is preferable to send your question publicly this way, because other people having the same question can find answer on the web. However, if your question is confidential, for example when it includes your unpublished data, please contact me by email. You can find my email address in <a href="https://doi.org/10.1002/vzj2.20168">this paper</a>.</p>'
    if ID == 'format':
        if lang == 'ja':
