...
''')
    ...
    for id in data:
        ...
        soil = d['Soil sample']
        texture = d['Texture']
        f.write('<h2>{0} : {1}</h2>\n<p><div align="center"><img src="img/{2}.svg" alt="Sample output"></div></p>'.format(soil, texture, id))
...
def table(models):
    text = '<table border="1"><tr><td>Abbr.<td>Model<td>Equation<td>Parameters</tr>'
    for id in models:
        m = model(id)
        text += f'<tr><td>{id}<td>{m['html']}<td>\\({m['equation']}\\)<td>&theta;<sub>s</sub> &theta;<sub>r</sub>, {(', ').join(m['parameter'])}</tr>'
    text += '</table>'
    return text
...
<li>Seki, K., Toride, N., & Th. van Genuchten, M. (2022). Closed-form hydraulic conductivity equations for multimodal unsaturated soil hydraulic properties. <i>Vadose Zone J.</i> 21, e20168. <a href="https://doi.org/10.1002/vzj2.20168">doi:10.1002/vzj2.20168</a></li>
<li>Seki, K., Toride, N., & Th. van Genuchten, M. (2023). Evaluation of a general model for multimodal unsaturated soil hydraulic properties. <i>J. Hydrol. Hydromech.</i> 71(1): 22-34. <a href="https://doi.org/10.2478/johh-2022-0039">doi:10.2478/johh-2022-0039</a>
<li>Seki, K., van Genuchten, M.T., Durner, W., Zhuang, L., Bermudez, L.B.S. (2026). Trimodal hydraulic models for unsaturated flow: Coupling triple-porosity water retention with general conductivity functions. <i>J. Hydrol. Hydromech.</i> <a href="https://researchmap.jp/sekik/published_papers/51967432/attachment_file.pdf">in press</a>.
'''
...
<h2 id="unimodal">Unimodal models</h2>
''')
    f.write(table(model('unimodal')))
    f.write('''
...
<div align="center"><img src="img/unimodal.svg" alt="Unimodal models"></div>

<h2 id="multimodal">Multimodal models</h2>
...
<h2 id="bimodal">Bimodal models</h2>
...
<div align="center"><img src="img/bimodal.svg" alt="Bimodal models"></div>
...
<div align="center"><img src="img/dual-ch.svg" alt="dual-CH models"></div>
...
<p>See also Figures 1 and 3 in Seki et al., 2026.</p>

<h2 id="conductivity">Hydraulic conductivity functions</h2>
...
<h2 id="unimodal">基本モデル（ユニモーダルモデル）</h2>
''')
    f.write(table(model('unimodal')))
    f.write('''
<ul>
...
<h2 id="multimodal">線型和モデル（マルチモデル）</h2>
...
<h2 id="bimodal">二重モデル（バイモーダルモデル）</h2>
...
<p>Seki et al. (2026) の Fig.1 と Fig.3 を参照。</p>

<h2 id="conductivity">透水性関数</h2>
